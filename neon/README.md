...
| Path | Purpose |
| --- | --- |
| `neon/evento-globolo-prod/terraform/` | pinned `kislerdm/neon` provider + lock file; project `evento-globolo-prod` in `aws-us-east-2` with `prevent_destroy` |
...
State: **planned**. No evento-globolo Neon organization or project has been provider-verified yet, so no ID in this repo is
...
- Migrations are authored in `evento-globolo/evgl-lib-core` and applied by `dpm`; this repository is not a migration authority.
