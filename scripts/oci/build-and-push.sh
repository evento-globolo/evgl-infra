...
login_registry() {
  case "$REGISTRY_PROVIDER" in
    ...
    dockerhub)
      ...
      printf '%s' "$DOCKERHUB_TOKEN" | docker login --username "$DOCKERHUB_USERNAME" --password-stdin "$REGISTRY_HOST"
      ;;
    gcp-artifact-registry)
      required REGISTRY_HOST
      gcloud auth print-access-token | docker login --username oauth2accesstoken --password-stdin "https://${REGISTRY_HOST}"
      ;;
    azure-acr)
