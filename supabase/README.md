# Supabase overlays for evento-globolo
...
**`evento_globolo`**. Every migration in these overlays must stay inside `evento_globolo`; the first migration creates it and
...
- **Authority does not move.** Portable SQL and JSON Schema stay in `evento-globolo/evgl-lib-core` (lib-core tier).
