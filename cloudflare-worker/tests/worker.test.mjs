...
function configuredHarness(overrides = {}) {
  ...
  const env = {
    ...
    STATE_KV: {
      put: async (key, value, options) => persisted.push([key, value, options]),
    },
    ...overrides,
  };
...
test('health response carries restrictive headers and no wildcard CORS', async () => {
  ...
  assert.equal((await response.json()).product, 'evento-globolo');
});
...
test('signed bodies still receive media-type and JSON-object validation', async () => {
  ...
  const malformed = await handleRequest(await signedRequest('/api/events', '{', env), env);
  assert.equal(malformed.status, 400);

  const arrayBody = await handleRequest(await signedRequest('/api/events', '[]', env), env);
...
test('server identity fields cannot be overwritten by an incoming event', async () => {
  ...
  const raw = JSON.stringify({
    ...
    product: 'not-evento-globolo',
    receivedAt: '1970-01-01T00:00:00.000Z',
    type: 'event.published',
  });
  const response = await handleRequest(await signedRequest('/api/events', raw, env), env);
  assert.equal(response.status, 202);
  const stored = JSON.parse(persisted[0][1]);
  assert.notEqual(stored.id, 'attacker-controlled-id');
  assert.equal(stored.product, 'evento-globolo');
  assert.notEqual(stored.receivedAt, '1970-01-01T00:00:00.000Z');
  assert.equal(stored.type, 'event.published');
});
