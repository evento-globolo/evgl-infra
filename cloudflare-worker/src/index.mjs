...
const PRODUCT = Object.freeze({
  org: 'evento-globolo',
  name: 'Evento Globolo',
  tagline: 'One global event system for discovery, management, automation, and cross-posting.',
  audience: 'event organizers, venues, communities, promoters, attendees, and integration partners',
  integrations: [
  ...
  capabilities: ['event-catalog', 'cross-posting', 'webhooks', 'oauth-connections', 'events', 'audit'],
});
...
function configuration(env = {}) {
  ...
  return { configured: Object.values(bindings).every(Boolean), bindings };
}
...
export function openApiDocument() {
  return {
    ...
    paths: {
      ...
      '/api/webhooks/{provider}': { post: signedOperation('Accept a signed provider webhook') },
    },
  };
}
...
export async function handleRequest(request, env = {}, ctx = {}) {
  ...
  if (url.pathname === '/healthz' && request.method === 'GET') {
    return json(request, { ok: true, product: PRODUCT.org, status: 'healthy', requestId }, {}, env);
  }
  ...
  if (provider && request.method === 'POST') {
    ...
    await schedule(ctx, enqueue(env.EVENT_QUEUE, { type: 'provider.webhook.received', webhook }));
    return json(
      request,
